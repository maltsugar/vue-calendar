<template>
    <div class="calendar-page">
        <div class="tool-base">
            <div class="left">
                <img class="q-btn" src="@/assets/img/cal_db_leftarrow.png" alt="" @click="handleChangBtnAction(-2)" />
                <img class="q-btn" src="@/assets/img/cal_leftarrow.png" alt="" @click="handleChangBtnAction(-1)" />
            </div>
            <div class="mid">{{ curTitle }}</div>
            <div class="right">
                <img class="q-btn" src="@/assets/img/cal_rightarrow.png" alt="" @click="handleChangBtnAction(1)" />
                <img class="q-btn" src="@/assets/img/cal_db_rightarrow.png" alt="" @click="handleChangBtnAction(2)" />
            </div>
        </div>
...
const props = defineProps({
    ...
    selectionType: { type: String, default: "range" },
})
...
const showWeekIdx = computed(() => {
    return props.weekIndexTitle?.length > 0
})
...
// 日期点击事件
function handleDayClick(day: calendarTools.DayConfig, week: calendarTools.WeekConfig) {
    if (day.isGray) {
        return
    }
    ...
    } else if (props.selectionType == "week") {
...
const touchmove = (event: TouchEvent) => {
    let _etouch = event.touches[0]
    touch.value.x = (_etouch.clientX - touchStartX) / (base.value?.offsetWidth ?? 1)
    touch.value.y = (_etouch.clientY - touchStartY) / (base.value?.offsetHeight ?? 1)
...
const reloadCalendarData = () => {
    ...
    monthA.value = calendarTools.getFullMonthDaysInDate(_dateA, props.weekStartDay)
    month0.value = calendarTools.getFullMonthDaysInDate(curDate.value, props.weekStartDay)
    monthB.value = calendarTools.getFullMonthDaysInDate(_dateB, props.weekStartDay)
...
const checkSelectedDate = (monthArr: calendarTools.WeekConfig[]) => {
    if (["range", "week"].includes(props.selectionType)) {
        // 重置所有选中状态
            ...
            for (const week of monthArr) {
                for (const day of week.days) {
                    if (
                        ...
                        day.isSelected = true
                        if (props.selectionType == "week" && !week.isSelected) {
